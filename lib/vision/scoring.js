...
export async function scoreOne(landmarks, canvas) {
  ...
  // Quality penalties — light (don’t crush scores)
...
async function normalizeWithAnchors(raw) {
  ...
  for (const k of ['symmetry','jawline','eyes','skin','balance']) {
    ...
    let pct   = clamp(cdfFromZ(z) * 10, 0, 10);

    // --- curved easing floor ---
    if (pct < 7) {
      let t = smooth01(pct / 7, 0, 1);   // smooth [0..7] → [0..1]
      const range = (k === 'skin') ? 0.9 : 1.1;
      pct = 7 + t * range;
    }
    // ---------------------------
