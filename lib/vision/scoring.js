...
export async function scoreOne(landmarks, canvas) {
  ...
  const overall   = clamp(base - pose * 0.9 - qualityPenalty * 0.5, 0, 10);
...
function computeRaw(landmarks, canvas) {
  ...
  // SKIN (variance around nose/cheek)
  let skin = 5.5;
  try {
...
/* ---------- normalization with anchors (and floors) ---------- */
async function normalizeWithAnchors(raw) {
  ...
  for (const k of ['symmetry','jawline','eyes','skin','balance']) {
    ...
    const pct = clamp(cdfFromZ(z) * 10, 0, 10);
    out[k]    = pct;
  }

  // 🔒 Floors to avoid buggy lows as requested
  if (out.skin    < 7) out.skin    = 7;
  if (out.balance < 7) out.balance = 7;
