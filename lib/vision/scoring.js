...
function computeRaw(landmarks, canvas) {
  ...
  // SYMMETRY (mirror distances about midline)
  ...
  // SKIN (luma variance patch around nose/cheek)
  let skin = 5.5;
  try {
...
async function normalizeWithAnchors(raw) {
  ...
  for (const k of ['symmetry','jawline','eyes','skin','balance']) {
    ...
    let pct   = clamp(cdfFromZ(z) * 10, 0, 10); // 0..10 baseline from CDF

    if (k === 'skin') {
      // Strict band with curved easing: [7.8, 9.8]
      const min = 7.8, max = 9.8;
      const t   = smooth01(pct / 10, 0, 1);     // 0..1 with smooth curve
      pct = min + t * (max - min);
    } else if (k === 'balance') {
      // Eased floor into [7.5, 9.8] (not hard cap unless > 9.8)
