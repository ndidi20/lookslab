...
export default function FaceOffStudio() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!alive) return;
        setMe({ loggedIn: false, pro: false, checking: false });
      }
  ...
  const exportCard = async () => {
    ...
    // watermark: subtle for Pro, strong for non‑Pro
    if (me.pro) {
  ...
  return (
    <Shell>
      ...
      {!modelsReady && <p className="text-sm text-neutral-400 mb-4">Loading models…</p>}
      ...
      <div className="mt-6 flex gap-3">
        <button
          onClick={analyze}
          disabled={!modelsReady || (!left.url && !right.url) || loading}
...
/* ---------- shared scoring (copy stays in this file for tree‑shaking) ---------- */
