...
export default function FaceOffStudio() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setLeft(s => ({ ...s, res: L }));
      setRight(s => ({ ...s, res: R }));
      if (!L && !R) alert('Couldn’t detect a face in either photo. Try higher light / larger crop.');
    } catch (e) {
  ...
  return (
    <Shell>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Slot label="Left"  s={left}  setS={setLeft}  onPick={pick('left')}  />
        <Slot label="Right" s={right} setS={setRight} onPick={pick('right')} />
      </div>

      <div className="mt-6 flex flex-wrap items-center gap-3">
        <button
          onClick={analyze}
          disabled={!ready || busy || (!left.url && !right.url) || !consent}
...
function Slot({ label, s, setS, onPick }) {
  return (
    <div className="rounded-lg border border-neutral-800 bg-black/40 p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          ...
          {s.url && (
            <button onClick={() => setS({ url:'', res:null })} className="px-3 py-1.5 rounded border border-neutral-700 hover:bg-neutral-900 text-sm">
              Clear
            </button>
          )}
        </div>
      ...
      <div className="aspect-[3/4] w-full rounded-md overflow-hidden bg-black/30 border border-neutral-900 flex items-center justify-center">
        {s.url ? <img src={s.url} alt="" className="w-full h-full object-contain" /> : <p className="text-neutral-500 text-sm">No image</p>}
      </div>

      {s.res && (
        <div className="mt-3 text-sm grid grid-cols-2 gap-1">
          <Row label="Overall" value={s.res.overall} />
          <Row label="Symmetry" value={s.res.breakdown.symmetry} />
          <Row label="Proportions" value={s.res.breakdown.proportions} />
          <Row label="Jawline" value={s.res.breakdown.jawline} />
          <Row label="Potential" value={s.res.potential} />
        </div>
      )}
    </div>
  );
...
/* ─── analysis helpers (shared with scan) ─────────────────────────────── */

async function analyzeOne(url, faceapi) {
  const img = await loadImg(url);
  const c = document.createElement('canvas'); c.width = 640; c.height = 480;
  const ctx = c.getContext('2d');
  const fit = fitContain(img.width, img.height, c.width, c.height);
  ctx.fillStyle = 'black'; ctx.fillRect(0,0,c.width,c.height);
  ctx.drawImage(img, (c.width-fit.w)/2, (c.height-fit.h)/2, fit.w, fit.h);
  ...
  return scoreFromLandmarks(best.landmarks);
}
